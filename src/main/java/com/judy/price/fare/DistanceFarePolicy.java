...
public class DistanceFarePolicy implements FarePolicy{
    ...
    public int getAdultFare(int distance){
        int fare=basicFare;  // 10km 이내
        int oneStep=10;
        int twoStep=50;
        int farePerDistance=100;

        if(distance > oneStep && distance <= twoStep){
            int extraFare = (int) Math.ceil( (double) (distance-oneStep) / 5 ) * farePerDistance;
            fare += extraFare;
        }else if(distance > twoStep){
            fare += (twoStep-oneStep)/5 * farePerDistance;
            int extraFare = (int) Math.ceil( (double) (distance-twoStep) / 8 ) * farePerDistance;
            fare += extraFare;
        }
        return fare;
    }
